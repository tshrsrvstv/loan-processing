...
def create_model():
    ...
    rand = RandomizedSearchCV(logreg, hyperparameters, random_state=42, n_iter=10, cv=10, n_jobs=-1,
...
def run_model(X, y, model_name, perform_smote=False):
    logger.info('In LogisticRegression | run_model started')
    ...
    print('Running Standard Logistic Regression on ' + model_name)
    std_model.fit(X_train, y_train)
    y_pred = std_model.predict(X_test)
    y_pred_train = std_model.predict(X_train)
    print('Confusion Martix for Standard Logistic Regression model on ' + model_name)
    print(confusion_matrix(y_test, y_pred))
    print("Training Accuracy for Standard Logistic Regression Model on " + model_name)
    print(metrics.accuracy_score(y_train, y_pred_train))
    print("Training Log loss for Standard Logistic Regression Model on " + model_name)
    print(metrics.log_loss(y_train, y_pred_train))
    print("Accuracy for Standard Logistic Regression Model on " + model_name)
    print(metrics.accuracy_score(y_test, y_pred))
    print("Log loss for Standard Logistic Regression Model on " + model_name)
    print(metrics.log_loss(y_test, y_pred))
    print("Classification Report for Standard Logistic Regression Model on " + model_name)
    ...
    plt.ion()
    plt.show()
    plt.savefig(os.path.join(VISUALIZATION_SAVE_DIRECTORY, 'Std_LR_' + model_name + '_roc_auc'))
    plt.pause(1)
    plt.close()
    print('Running RandomizedSearch Logistic Regression on ' + model_name)
    randomize_best_model = randomize_model.fit(X_train, y_train)
    print('Best Penalty:', randomize_best_model.best_estimator_.get_params()['penalty'])
    print('Best C:', randomize_best_model.best_estimator_.get_params()['C'])
    print('Best solver:', randomize_best_model.best_estimator_.get_params()['solver'])
    y_pred = randomize_best_model.predict(X_test)
    y_pred_train = randomize_best_model.predict(X_train)
    print('Confusion Martix for Best Randomized Logistic Regression model on ' + model_name)
    print(confusion_matrix(y_test, y_pred))
    print("Training Accuracy for Best Randomized Logistic Regression Model on " + model_name)
    print(metrics.accuracy_score(y_train, y_pred_train))
    print("Training Log Loss for Best Randomized Logistic Regression Model on " + model_name)
    print(metrics.log_loss(y_train, y_pred_train))
    print("Accuracy for Best Randomized Logistic Regression Model on " + model_name)
    print(metrics.accuracy_score(y_test, y_pred))
    print("Log Loss for Best Randomized Logistic Regression Model on " + model_name)
    print(metrics.log_loss(y_test, y_pred))
    print("Classification Report for Randomized Logistic Regression Model on " + model_name)
    print(classification_report(y_test, y_pred))
    ...
    plt.ion()
    plt.show()
    plt.savefig(os.path.join(VISUALIZATION_SAVE_DIRECTORY, 'Randomized_LR_' + model_name + '_roc_auc'))
    plt.pause(1)
    plt.close()
    logger.info('In LogisticRegression | run_model finished')
